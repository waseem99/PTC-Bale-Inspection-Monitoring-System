...
import { AppShell, EmptyState, Freshness, Icon, LoadingPanel, Pagination, QueryErrorState, StatusPill, outcomeTone, reviewTone, useToast } from '../components';
...
export default function EventsPage() {
  ...
  return <AppShell title="Events">
    ...
    <section className="panel table-panel">{events.isLoading && <LoadingPanel rows={8} label="Loading event table"/>}<QueryErrorState error={events.error} hasData={events.data !== undefined} onRetry={() => void events.refetch()}/> {events.data && events.data.items.length === 0 && <EmptyState title="No matching events" message="Adjust the filters or date range and try again."/>}
