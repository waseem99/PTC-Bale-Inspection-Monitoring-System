...
import { AppShell, ErrorState, Freshness, Icon, LoadingPanel, StatusPill } from '../components';
...
export default function LivePage() {
  ...
  return <AppShell title="Live Monitoring">
    ...
    <div className="info-banner"><Icon name="camera" size={20}/><span>Configured recorded-video previews are shown per camera when available. Cameras without a configured preview keep the synthetic fallback until the approved live stream is connected.</span></div>
    {cameras.isLoading && <LoadingPanel rows={6} label="Loading live monitoring"/>}{cameras.error !== undefined && !cameras.data && <ErrorState error={cameras.error} onRetry={() => void cameras.refetch()}/>} 
    {cameras.data && <section className="live-camera-grid" aria-label="Camera monitoring grid">{cameras.data.map((camera) => <article className="live-camera-card" key={camera.id}><CameraPreview camera={camera}/><div className="live-camera-card__header"><div><strong>{camera.name}</strong><span>{camera.zone}</span></div><StatusPill label={camera.status} tone={camera.status === 'online' ? 'good' : camera.status === 'warning' ? 'warn' : 'bad'}/></div><dl className="camera-metrics"><div><dt>AI</dt><dd>{camera.aiStatus}</dd></div><div><dt>Last frame</dt><dd>{formatRelativeTime(camera.lastFrameAt)}</dd></div><div><dt>Frame rate</dt><dd>{camera.fps} FPS</dd></div><div><dt>Stream</dt><dd>{camera.streamQuality}</dd></div><div><dt>Events today</dt><dd>{camera.todayEvents}</dd></div></dl></article>)}</section>}
  </AppShell>;
}
