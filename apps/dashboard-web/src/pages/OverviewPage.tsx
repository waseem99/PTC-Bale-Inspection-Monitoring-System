...
import { AppShell, CameraScene, ErrorState, Freshness, Icon, LoadingPanel, QueryErrorState, StatusPill, outcomeTone, reviewTone } from '../components';
...
export default function OverviewPage() {
  ...
  return <AppShell title="Overview">
    ...
    {summary.isLoading && <LoadingPanel rows={4} label="Loading dashboard summary"/>}<QueryErrorState error={summary.error} hasData={summary.data !== undefined} onRetry={() => void summary.refetch()}/>
    ...
    <section className="dashboard-grid"><article className="panel panel--wide"><div className="panel__header"><div><span className="eyebrow">Camera network</span><h3>Live monitoring status</h3></div><Link to="/live">View all cameras <Icon name="arrow" size={17}/></Link></div>
      {cameras.isLoading && <LoadingPanel rows={3} label="Loading camera status"/>}<QueryErrorState error={cameras.error} hasData={cameras.data !== undefined} onRetry={() => void cameras.refetch()}/>
      ...
      {recentEvents.isLoading && <LoadingPanel rows={5} label="Loading recent events"/>}<QueryErrorState error={recentEvents.error} hasData={recentEvents.data !== undefined} onRetry={() => void recentEvents.refetch()}/>
