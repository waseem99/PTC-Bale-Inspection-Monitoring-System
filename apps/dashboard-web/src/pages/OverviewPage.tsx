...
import { AppShell, ErrorState, Freshness, Icon, LoadingPanel, StatusPill, outcomeTone, reviewTone } from '../components';
...
export default function OverviewPage() {
  ...
  return <AppShell title="Overview">
    ...
    {summary.isLoading && <LoadingPanel rows={4} label="Loading dashboard summary"/>}{summary.error !== undefined && !summary.data && <ErrorState error={summary.error} onRetry={() => void summary.refetch()}/>} 
    ...
    <section className="dashboard-grid"><article className="panel panel--wide"><div className="panel__header"><div><span className="eyebrow">Camera network</span><h3>Live monitoring status</h3></div><Link to="/live">View all cameras <Icon name="arrow" size={17}/></Link></div>
      {cameras.isLoading && <LoadingPanel rows={3} label="Loading camera status"/>}{cameras.error !== undefined && !cameras.data && <ErrorState error={cameras.error} onRetry={() => void cameras.refetch()}/>} 
      {cameras.data && <div className="camera-mini-grid">{cameras.data.map((camera) => <article className="camera-mini-card" key={camera.id}><CameraPreview camera={camera}/><div className="camera-card__body"><div><strong>{camera.name}</strong><span>{camera.zone}</span></div><StatusPill label={camera.status} tone={camera.status === 'online' ? 'good' : camera.status === 'warning' ? 'warn' : 'bad'}/></div></article>)}</div>}
      ...
      {recentEvents.isLoading && <LoadingPanel rows={5} label="Loading recent events"/>}{recentEvents.error !== undefined && !recentEvents.data && <ErrorState error={recentEvents.error} onRetry={() => void recentEvents.refetch()}/>} 
