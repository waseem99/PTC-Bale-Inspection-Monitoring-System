...
function mockUsers(): Array<User & { password: string }> { const password = import.meta.env.VITE_DEMO_PASSWORD ?? (import.meta.env.DEV ? 'PTC-Demo-2026!' : 'mock-password-not-configured'); return [ { id: 'usr-supervisor', username: 'supervisor', displayName: 'Supervisor Demo', role: 'supervisor', password }, { id: 'usr-viewer', username: 'viewer', displayName: 'Viewer Demo', role: 'viewer', password }, { id: 'usr-admin', username: 'admin', displayName: 'Administrator Demo', role: 'admin', password } ]; }
...
export function requireMockSession(token: string | undefined): Session { if (!token) { window.dispatchEvent(new Event('ptc:auth-expired')); throw new ApiError({ code: 'UNAUTHENTICATED', message: 'Sign in to continue.', status: 401, correlationId: randomCorrelationId() }); } const session = decodeMockSession(token); if (!session) { window.dispatchEvent(new Event('ptc:auth-expired')); throw new ApiError({ code: 'SESSION_EXPIRED', message: 'Your session has expired. Sign in again.', status: 401, correlationId: randomCorrelationId() }); } return session; }
