...
export function QueryErrorState({ error, hasData, onRetry }: { error?: Error | undefined; hasData: boolean; onRetry: () => void }): ReactNode {
  if (error == null || hasData) return null;
  return <ErrorState error={error} onRetry={onRetry} />;
}
export function InlineErrorMessage({ error, fallback = 'An unexpected error occurred.' }: { error?: Error | undefined; fallback?: string }): ReactNode {
  if (error == null) return null;
  return <p className="form-error" role="alert">{error.message || fallback}</p>;
}
