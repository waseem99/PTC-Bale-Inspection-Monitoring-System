...
5. local Node.js API, PostgreSQL persistence, evidence storage, and approved Azure management plane;
...
| Week | Scope and client decisions | Site and edge | AI | Local React/Node/PostgreSQL application and approved Azure | Test and release |
|---|---|---|---|---|---|
| 1 | Complete #59 reference mapping; confirm PTC SOP, terminology, violations, local/Azure boundary, acceptance scenarios, and retention decisions | Complete site survey; compare overhead/oblique/side views; prepare camera plan | Define PTC data collection, annotation, and reference-data rules | Define API contracts, PostgreSQL schema, local access, deployment modes, migration/backup approach, and approved Azure prerequisites | Create test strategy, PoC stage gates, and baseline backlog |
| 2 | Approve camera positions, hardware BOM, and reference-to-PTC exclusions | Install/configure hardware or prepare final installation; begin PTC footage collection | Curate initial PTC dataset | Initialize Node API, React dashboard, Prisma/PostgreSQL schema and migration, edge deployment scripts, conditional Bicep, and CI | Hardware, field-of-view, migration and stream acceptance begins |
| 3 | Close outstanding physical workflow and exception decisions | Implement RTSP ingest, orientation, camera health, zones, and rolling buffer | Annotate PTC dataset and train baseline detector | Implement API, relational constraints/indexes, fixed access, seed tooling, and dashboard shell | Unit, API, PostgreSQL and component tests |
| 4 | Review baseline system behavior against the approved PTC workflow | Complete local spool, local evidence clips, and service packaging | Implement tracking and initial worker-bale association | Implement local event ingestion, protected evidence metadata, PostgreSQL persistence, transactional review/audit, and list/detail APIs | First end-to-end recorded-PTC-footage flow |
| 5 | Validate reason codes and review workflow | Stabilize reconnect and service recovery | Implement opening/frisking interaction signals and SOP state machine | Build event table, detail, filters, evidence access, local real-time channel, and health | Scenario testing, AI error review and query-performance checks |
| 6 | Confirm final on-demand reporting and access scope | Integrate local application persistence and optional approved synchronization | Tune violation rules and export model runtime | Complete review status, remarks, exports, local access, health, PostgreSQL backup/restore, and approved Azure management plane | Security, offline resilience, migration and performance testing |
| 7 | Prepare UAT users, schedule, and locked PTC scenarios | Deploy full local stack on site | Live PTC calibration and threshold tuning | Deploy local UAT application/PostgreSQL; deploy approved Azure components where applicable | End-to-end site testing; close P0/P1 defects |
| 8 | Conduct UAT and record PoC decision | Final local production configuration and release package | Freeze PoC model/rules/configuration | Final approved local/hybrid deployment, migration status, backup, documentation, and training | UAT, release notes, handover, `v1.0.0-mvp` |
...
| 3 | Performance, evidence, storage, PostgreSQL, backup, synchronization, and configuration tuning |
...
- PostgreSQL/Prisma decision recorded under ADR-012;
...
### M4 — Local React/Node/PostgreSQL application and approved Azure management plane ready

- locally deployed Node.js/Express event ingestion and idempotency;
- local PostgreSQL persistence, foreign keys, constraints and indexes;
- Prisma schema and committed migration history;
- local protected evidence storage and safe metadata;
- approved local access control;
- filtering, review, remarks, exports, local real-time updates, and health;
- database restart persistence and `pg_dump`/`pg_restore` test;
- optional approved Azure Database for PostgreSQL/Blob/Entra/monitoring components;
...
- PostgreSQL migration, persistence and recovery validated;
...
- approval of PostgreSQL backup location, retention and recovery ownership;
- Azure subscription, identity, networking, Azure Database for PostgreSQL, and deployment access where required;
- client review turnaround;
- hardware, camera-angle, or lighting changes after the site survey.

## Change control

A requirement that affects scope, architecture, camera quantity, scanner/IoT/external integration, retention, accuracy commitments, database technology/schema, technology standards, or delivery timeline must be:
