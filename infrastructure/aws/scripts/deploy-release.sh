...
rollback() {
  ...
  if [[ -n "$PREVIOUS_RELEASE" && -d "$PREVIOUS_RELEASE" ]]; then
    echo "Rolling application containers back to ${PREVIOUS_RELEASE}; database migrations require forward recovery."
...
[[ -s "${SHARED_DIR}/runtime.env.new" ]] || { echo "Decoded runtime environment is empty." >&2; exit 1; }
...
for attempt in {1..60}; do
  if docker compose -f docker-compose.staging.yml exec -T postgres \
    ...
    -d "$(grep '^POSTGRES_DB=' .env | cut -d= -f2-)" >/dev/null 2>&1; then break; fi
...
for attempt in {1..60}; do
  ...
  if [[ "$API_HEALTH" == "healthy" && "$DASHBOARD_HEALTH" == "healthy" ]]; then break; fi
  if [[ "$attempt" -eq 60 ]]; then
